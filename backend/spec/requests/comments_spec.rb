...
RSpec.describe 'CommentsController' do
  ...
  describe 'PUT /memos/:memo_id/comments/:id' do
    context 'コンテンツが有効な場合' do
      ...
      it 'コメントが更新され、204になる' do
        aggregate_failures do
          put "/memos/#{memo.id}/comments/#{comment.id}", params: { comment: valid_comment_params }, as: :json
          expect(response).to have_http_status(:no_content)
          expect(response.body).to be_empty
        end
      end
  ...
  context '存在しないコメントIDの場合' do
    ...
    it '404が返ることを確認する' do
      aggregate_failures do
        put "/memos/#{memo.id}/comments/0", params: { comment: valid_comment_params }, as: :json
  ...
  context '存在しないメモIDの場合' do
    ...
    it '404が返ることを確認する' do
      aggregate_failures do
        put "/memos/0/comments/#{comment.id}", params: { comment: valid_comment_params }, as: :json
        expect(response).to have_http_status(:not_found)
      end
    end
  end
